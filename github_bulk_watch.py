...
TOP_N = int(os.environ.get("TOP_N", "50000"))
...
#   Her grup bitince state DISKE KAYDEDILIR (asagidaki kritik duzeltmeye
#   bak) ve kisa bir mola verilir. 50.000 repo'yu tek seferde thread'e
#   atmak yerine boyle bolmek hem daha guvenli hem de yarida kesilirse
#   kaybi minimuma indiriyor.

BATCH_PAUSE_SECONDS = float(os.environ.get("BATCH_PAUSE_SECONDS", "2"))
...
def request_with_retry(method, url, retries=5, **kwargs):
    ...
    for attempt in range(retries):
        ...
        if 500 <= r.status_code < 600 and attempt < retries - 1:
            safe_print(f"   🔁 Geçici sunucu hatası (HTTP {r.status_code}), {backoff}s sonra tekrar denenecek...")
            time.sleep(backoff)
...
def search_bucket(query, max_items=1000):
    ...
    (repos, basarili_mi) tuple'i donduruyor - boylece cagiran taraf,
    sorgu GERCEKTEN basarili mi yoksa hata yuzunden mi bos donduyu
    ayirt edip, sadece basarili olani state'e 'tarandi' diye
    isaretleyebiliyor."""
    repos = []
    page = 1
    per_page = 100
...
def search_bucket_resilient(query, attempts=2):
    """EKLENDI: search_bucket basarisiz olursa (rate limit disinda bir
    hata, mesela GitHub tarafinda gecici bir 5xx), pes etmeden kisa bir
    bekleme ile bu kadar kez daha dener - 'kendisi baska yol denesin'
    istegi burada karsiliyor. Hala basarisizsa (ok=False, []) doner,
    checkpoint yapilmaz, bir sonraki tam calistirmada (resume) tekrar
    denenir."""
    for attempt in range(attempts):
        ...
        if attempt < attempts - 1:
            safe_print(f"   🔁 '{query}' sorgusu için {attempt + 1}. deneme başarısız, kısa bekleyip tekrar denenecek...")
            time.sleep(3)
    return found, ok
    ...
    if INCLUDE_STAR_SEARCH:
        ...
        for i, bucket in enumerate(STAR_BUCKETS, start=1):
            ...
            if query in STATE["queries_done"]:
                ...
                found, ok = search_bucket_resilient(query)
                add_repos(found)
                if ok:
                    checkpoint(query)
                else:
